...
class ShakePyramidNet(nn.Module):
    ...
    def __init__(self, depth=110, alpha=270, label=10, gpu:int=-1):
        ...
        self.in_channels, self.u_idx = in_channels, 0
        self.ps_shakedrop = [1 - (1.0 - (0.5 / (3*n_units)) * (i+1)) for i in range(3*n_units)]

        self.c_in = nn.Conv2d(3, in_channels[0], 3, padding=1)
        self.bn_in = nn.BatchNorm2d(in_channels[0])
        ...
        self.bn_out = nn.BatchNorm2d(in_channels[-1])
        self.fc_out = nn.Linear(in_channels[-1], label)
